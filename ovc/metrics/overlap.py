...
def overlap_metrics(overlaps_metric: gpd.GeoDataFrame) -> dict:
    ...
    total = (
        float(overlaps_metric["inter_area_m2"].sum())
        if "inter_area_m2" in overlaps_metric.columns
        else float(overlaps_metric.geometry.area.sum())
    )
    count = int(len(overlaps_metric))
    avg = float(total / count) if count else 0.0

    vc = (
        overlaps_metric["overlap_type"].value_counts().to_dict()
        if "overlap_type" in overlaps_metric.columns
        else {}
    )
    return {
        "overlap_count": count,
